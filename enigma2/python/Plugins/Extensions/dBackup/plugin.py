...
class dBackupChecking(Screen):
	...
	if boxtype != "dm520":
		...
		if os.path.exists("/dev/disk/by-label/dreambox-data"):
		       	flashchecklist.append((_("check & repair recovery"), "umount /dev/disk/by-label/dreambox-data; /sbin/fsck.ext4 -f -v -y /dev/disk/by-label/dreambox-data")) 
			if os.path.exists("/sbin/badblocks"):
		        	flashchecklist.append((_("badblocks recovery > 1min"), "umount /dev/disk/by-label/dreambox-data; /sbin/fsck.ext4 -f -c -v -y /dev/disk/by-label/dreambox-data")) 
			else:
		        	flashchecklist.append((_("no badblocks binary - get e2fsprogs"), "none")) 
		else:
