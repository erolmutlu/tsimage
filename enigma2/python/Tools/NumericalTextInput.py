# -*- coding: UTF-8 -*-
# colombo555 fix arabic language 
...
class NumericalTextInput:
	def __init__(self, nextFunc=None, handleTimeout = True, search = False):
		...
		if self.lang == 'de_DE':
			...
			self.mapping.append (u" 1.؛:/-_") 
			self.mapping.append (u"بتةث2") 
			self.mapping.append (u"اىءؤئ3")
			self.mapping.append (u"سشصض4") 
			self.mapping.append (u"دذرز5")   
			self.mapping.append (u"جحخ6") 
			self.mapping.append (u"نهوي7") 
			self.mapping.append (u"فقكلم8") 
			self.mapping.append (u"طظعغ9") 
	...
	def getKey(self, num):
		...
		while True:
			   ...
			   self.pos -= 1
			   if -1 >= self.pos:
				self.pos = len(self.mapping[num])-1	
                        else:
                           self.pos += 1
			...
			if self.useableChars:
                                if self.lang == 'ar_AE':
                                    pos = self.useableChars.find(self.mapping[num][len(self.mapping[num]) - self.pos -1])
                                else:
			...
			break
                if self.lang == 'ar_AE':
		   return self.mapping[num][len(self.mapping[num]) - self.pos -1]
                else:
